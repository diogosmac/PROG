...
string Board::getPosition(string word) {
	for (auto elem : boardKey.getMap()) {
		string posword = elem.second;
		if (posword == word) {
...
vector<pair<unsigned, unsigned>> Board::getBlackSquares() {
	vector<pair<unsigned, unsigned>> lockedSpots;
	for (unsigned i = 1; i <= dim_Y; i++) {
		for (unsigned j = 1; j <= dim_X; j++) {
			if (matrix[j][i] == '#')
				lockedSpots.push_back(make_pair(j, i));
		}
	}
	return lockedSpots;
}

void Board::lockBlackSquares(vector<pair<unsigned, unsigned>> blackSquares) {
	for (auto elem : blackSquares) {
		unsigned x, y;
		x = elem.first;
		y = elem.second;
		matrix[x][y] = '#';
...
void Board::boardUpdate_v2() {
	vector<pair<unsigned, unsigned>> blackSq = getBlackSquares();
	boardReset();
	lockBlackSquares(blackSq);
	map<string, string> wl = boardKey.getMap();
	for (auto elem : wl) {
		string pos, word;
		pos = elem.first;
